...
const {
  RUNNING_MODE_TYPES: RunningModeTypes,
  CLOUD_MODE_TYPES: CloudModeTypes,
} = require("./constants");
const fs = require("fs");
...
class EntryServer extends EventEmitter {
  ...
  addRoomId(roomId) {
    ...
    if (this.runningMode === RunningModeTypes.server) {
      ...
      printLog(
        "client roomId inserted. client will request match browser target:",
        roomId
      );
      this.clientRoomId = roomId;
  ...
  close() {
    ...
    this.httpServer && this.httpServer.listening && this.httpServer.close();
  ...
  _getSSLFileList() {
    ...
    if (isAllExists.every((result) => result)) {
      const [key, cert, ...ca] = SSLFileList.map((fileName) =>
    ...
    if (req.url.startsWith("/modules")) {
      ...
      if (!name) {
        ...
        if (handleModuleListRequest) {
          const moduleList = await handleModuleListRequest();
  ...
  _initServer(port = this.PORT) {
    ...
    try {
      const SSLFileList = this.options.http
        ? undefined
        : this._getSSLFileList();
  ...
  _initSocketServer() {
    ...
    socketServer.on("connection", (socket) => {
      ...
      connection.on("matchTarget", (roomIdArgument) => {
        const roomId =
          typeof roomIdArgument === "object"
            ? roomIdArgument.roomId
            : roomIdArgument;
  ...
  _removeHardwareClient(target) {
    ...
    this.hardwareClientList = this.hardwareClientList.filter(
      (client) => client.id !== target.id
    );
    if (target.connected) {
      target.disconnect();
    }
  ...
  _removeBrowserClient(target) {
    printLog(target.id, "is removed from list");
    this.browserClientList = this.browserClientList.filter(
      (client) => client.id !== target.id
    );
    if (target.connected) {
      target.disconnect();
